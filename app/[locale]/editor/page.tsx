...
export default async function EditorPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <><SiteHeader locale={locale} labels={t}/><main className="editorPage"><JsonLd data={jsonLd}/><div className="editorIntro"><p className="eyebrow">LOCAL IMAGE TOOL</p><h1>{t.editorTitle}</h1><p>{t.privateText}</p></div><OverlayEditor labels={t}/><AdSlot label={t.ad} slot={process.env.NEXT_PUBLIC_ADSENSE_EDITOR_SLOT}/><section className="guideSection"><h2>{t.guideTitle}</h2><p>{t.guideBody}</p><Link href={`/${locale}/how-it-works/`}>{t.navGuide} →</Link></section></main><SiteFooter locale={locale}/></>;
}
