...
export function SiteHeader({ locale, labels }: { locale: Locale; labels: (typeof copy)[Locale] }) {
  ...
  return <header className="siteHeader"><Link className="brand" href={`/${locale}/`}>{labels.siteName}</Link><nav aria-label="Primary navigation">
    <Link href={`/${locale}/editor/`}>{labels.navEditor}</Link><Link href={`/${locale}/how-it-works/`}>{labels.navGuide}</Link><Link href={`/${locale}/guides/`}>{labels.navGuides}</Link><Link href={`/${locale}/resources/`}>{labels.navResources}</Link><Link href={`/${locale}/about/`}>{labels.navAbout}</Link>
